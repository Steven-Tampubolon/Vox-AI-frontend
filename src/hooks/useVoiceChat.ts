...
export function useVoiceChat() {
    ...
    const [secondsLeft, setSecondLeft] = useState(MAX_RECORDING_SECONDS)
    ...
    const playAudio = useCallback((base64: string, mimeType: string) => {
        ...
        audio.onerror   = () => {
            toast.error("Gagal memuat audio AI")
            setIsPlaying(false)
        }
    ...
    const sendVoiceMessage = useCallback(async (
        ...
        try {
            ...
            const isNetwork = msg.includes("Failed to fetch") || msg.includes("NetworkError") || msg.includes("Networ Error")
            if (isNetwork) {
                toast.error("Tidak dapat terhubung ke server", {
                    description: "Pastikan backend berjalan di localhost:8000",
                })
    ...
    const stopRecording = useCallback(() => {
        ...
        setSecondLeft(MAX_RECORDING_SECONDS)
    }, [sendVoiceMessage])
